jQuery(function ($) {
    ...
    var api = tb_app,
		settings=null;
    var RunEditAAP = function(el,model){
		var wrap = el.getElementsByClassName('tbp_advanchd_archive_wrap')[0],
			...
			items = $('.'+arhiveCL),
			args=['order','orderby','offset','meta_key'], // parameters posted along with "tb_render_element" ajax
		...
		$(el).one('tb_layout_part_before_init',function(){
			...
			closeBtn.on('click',function(e){
				...
				if(isSaved===true || api.builderIndex===0){
					...
					document.body.classList.remove( 'tbp_module_' + model.get('mod_name') );
					window.top.document.body.classList.remove( 'tbp_app_is_edit' );
					window.top.document.body.classList.remove( 'tbp_module_' + model.get('mod_name') );
	...
	Themify.body.one('builder_load_module_partial', function(){
