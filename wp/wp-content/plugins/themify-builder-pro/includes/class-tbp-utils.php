...
class Tbp_Utils {
	...
	$args = array(
	    ...
	    'posts_per_page' => 1,
	    'no_found_rows' => true,
	    'post_status'=> !empty($_GET['preview']) ? 'any' : 'publish'
	);
	$cache=false;
	if(Themify_Builder::$frontedit_active===true || Tbp_Public::$isTemplatePage===true){
	    if($res===null){
			$type = isset( $_POST['type'] ) ? sanitize_text_field( $_POST['type'] ) : '';
			if ( isset( $_POST['aap_preview'] ) ) {
				// Advanced Posts module
				foreach( array( 'post_type', 'order', 'orderby', 'offset' ) as $v ) {
					if ( ! empty( $_POST[ $v ] ) ) {
						$args[ $v ] = sanitize_text_field( $_POST[ $v ] );
					}
				}
				if ( ! empty( $type ) ) {
					Themify_Builder_Model::parseTermsQuery( $args, $_POST['terms'], $type );
				}
				...
				elseif(taxonomy_exists($type)){
					if($isTerm===true){
						...
						if ( $_POST['term_type'] === 'post_slug'  ) {
							if($_POST['slug']!==''){
								$args['post__in'] = Themify_Builder_Model::parse_slug_to_ids( $_POST['slug'], $args['post_type'] );
							}
						} else {
							Themify_Builder_Model::parseTermsQuery( $args, $_POST['terms'], $type );
						}
	    ...
	    $args['post_status'] = ( ! empty( $args['p'] ) && 'attachment' === get_post_type( $args['p'] ) ) ? 'inherit' : $args['post_status'];
	}
	if($args['orderby']==='id'){
	    $args['orderby']='ID';
	}
		...
		if(Themify_Builder::$frontedit_active===true){
			...
			if ( isset( $_POST['aap_preview'] ) ) {
				...
				if ( ! empty( $_POST['type'] ) ) {
					...
					Themify_Builder_Model::parseTermsQuery( $args, $terms, 'product_cat' );
				}
